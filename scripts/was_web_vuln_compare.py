...
def _find_existing_header(df: pd.DataFrame) -> int:
    for i, row in df.iterrows():
        ...
        if "영역" in vals and "진단 항목" in vals:
            return i
    raise SystemExit("HEADER_NOT_FOUND: WAS_WEB_기존.xlsx")
...
def main() -> None:
    ...
    ap.add_argument("--ai-dir", required=True, help="Folder containing Mobile_* and WAS_WEB_기존.xlsx")
    ap.add_argument("--out-dir", required=True, help="Output folder for WAS_WEB_결과.xlsx")
    args = ap.parse_args()
    ...
    existing_path = ai_dir / "WAS_WEB_기존.xlsx"
    mobile_files = [
        ai_dir / "Mobile_WEB.xlsx",
        ai_dir / "Mobile_HJ_WAS.xlsx",
        ai_dir / "Mobile_Internet_WAS.xlsx",
    ]
    mobile_files = [p for p in mobile_files if p.exists()]
    if not mobile_files:
        raise SystemExit("NO_MOBILE_FILES: Mobile_WEB/HJ_WAS/Internet_WAS not found")
    ...
    local_out = Path("/tmp/WAS_WEB_결과.xlsx")
    write_output(existing, new_df, local_out)

    out_path = out_dir / "WAS_WEB_결과.xlsx"
    try:
        shutil.copy2(local_out, out_path)
    except PermissionError:
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        fallback = out_dir / f"WAS_WEB_결과_{ts}.xlsx"
