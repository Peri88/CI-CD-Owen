...
def _find_existing_header(df: pd.DataFrame) -> int:
    for i, row in df.iterrows():
        ...
        if "영역" in vals and "진단 항목" in vals:
            return i
    raise SystemExit("HEADER_NOT_FOUND: dbms_기존.xlsx")
...
def main() -> None:
    ...
    ap.add_argument("--ai-dir", required=True, help="Folder containing Mobile_*_DBMS and dbms_기존.xlsx")
    ap.add_argument("--out-dir", required=True, help="Output folder for dbms_결과.xlsx")
    args = ap.parse_args()
    ...
    existing_path = ai_dir / "dbms_기존.xlsx"
    mobile_files = sorted(ai_dir.glob("Mobile_*_DBMS.xlsx"))
    if not mobile_files:
        raise SystemExit("NO_MOBILE_FILES: Mobile_*_DBMS.xlsx not found")
    ...
    local_out = Path("/tmp/dbms_결과.xlsx")
    write_output(existing, new_df, local_out)

    out_path = out_dir / "dbms_결과.xlsx"
    try:
        shutil.copy2(local_out, out_path)
    except PermissionError:
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        fallback = out_dir / f"dbms_결과_{ts}.xlsx"
