...
def _find_existing_header(df: pd.DataFrame) -> int:
    for i, row in df.iterrows():
        ...
        if "영역" in vals and "진단 항목" in vals:
            return i
    raise SystemExit("HEADER_NOT_FOUND: Windows_기존.xlsx")
...
def collect_new(mobile_file: Path, existing_codes: set[str]) -> pd.DataFrame:
    new_rows = []
    xls = pd.ExcelFile(mobile_file)
    for sheet in xls.sheet_names:
        if sheet == "요약":
            continue
        df = pd.read_excel(mobile_file, sheet_name=sheet, header=None)
        hdr_idx = None
        for i, row in df.iterrows():
...
def main() -> None:
    ...
    ap.add_argument("--ai-dir", required=True, help="Folder containing Mobile_Internet_Windows.xlsx and Windows_기존.xlsx")
    ap.add_argument("--out-dir", required=True, help="Output folder for Windows_결과.xlsx")
    args = ap.parse_args()
    ...
    existing_path = ai_dir / "Windows_기존.xlsx"
    mobile_file = ai_dir / "Mobile_Internet_Windows.xlsx"

    existing = load_existing(existing_path)
    existing_codes = set(existing["Code"].dropna())
    new_df = collect_new(mobile_file, existing_codes)

    local_out = Path("/tmp/Windows_결과.xlsx")
    write_output(existing, new_df, local_out)

    out_path = out_dir / "Windows_결과.xlsx"
    try:
        shutil.copy2(local_out, out_path)
    except PermissionError:
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        fallback = out_dir / f"Windows_결과_{ts}.xlsx"
